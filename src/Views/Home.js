...
class Home extends Component {
    ...
    setOrgRepos = async () => {
            ...
            const link = "https://api.github.com/users/" + orgs[index].login + "/repos";
    ...
    render() {
        ...
        const organizations = orgs.map((item, index) => (
            <View>
                <Text style={{color: "#000", fontWeight: "bold", fontSize: 22, marginVertical: 10}}>{item.login}</Text>
        ...
        return (
                ...
                <TextInput
                    placeholder="search"
                    style={{marginVertical: 20, marginHorizontal: 10, padding: 10, borderRadius: 5, borderWidth: 1, borderColor: "#000", backgroundColor: "#fff", color: "#000"}}
                    autoCapitalize="none"
                    autoCorrect={false}
                />
                <View style={{flex: 1, backgroundColor: "#f7f7f7", paddingHorizontal: 10, paddingVertical: 20}}>

                    <Text style={{color: "#000", fontWeight: "bold", fontSize: 22, marginBottom: 10}}>Your Repositories</Text>
                    ...
                    <FlatList
                        ...
                        renderItem={
                            ({item}) => {
                                return (
                                    <ListItem item={item} onPress={() => this._OnItemPress(item.id)}/>
                                );
                            }
                        }
